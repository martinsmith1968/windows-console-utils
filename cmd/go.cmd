...
SET FOLDER=.
SET NAMEFILTER=
SET EXTENSIONCOUNT=4
SET EXTENSION1=exe
SET EXTENSION2=cmd
SET EXTENSION3=bat
SET EXTENSION4=com
SET EXTENSIONCALLPREFIX2=CALL
SET EXTENSIONCALLPREFIX3=CALL
SET ARGS=
...
IF %TARGETCOUNT% GTR 1 (
    ...
    TYPE "%UNIQUETEMPFILENAME% | CUT -d ":" -f 1
    GOTO :EOF
)
...
FOR /f "tokens=1-2 delims=:" %%A in ("%TARGETFILEDETAILS%") DO (
  SET TARGETFILENAME=%%A
  SET TARGETCALLPREFIX=%%B
)

IF "%VERBOSE%" == "Y" ECHO.Executing: %TARGETFILENAME% %ARGS%
@IF "%DEBUG%" == "Y" @ECHO ON
%COMMANDPREFIX%%TARGETCALLPREFIX% %TARGETFILENAME% %ARGS%
@IF "%DEBUG%" == "Y" @ECHO OFF
...
:FINDTARGET
SET TARGETCOUNT=0
SET TARGETFILEDETAILS=

PUSHD "%FOLDER%"

CALL BUILDUNIQUETEMPFILENAME "%SCRIPTNAME%.tmp"

FOR /L %%N IN (1, 1, %EXTENSIONCOUNT%) DO CALL :BUILDCANDIDATEFILES !EXTENSION%%N! !EXTENSIONCALLPREFIX%%N!
...
FOR /F %%F IN (%UNIQUETEMPFILENAME%%) DO (
    ...
    SET /A TARGETCOUNT += 1
    IF "!TARGETFILEDETAILS!" == "" SET TARGETFILEDETAILS=%%~F
)
@IF "%DEBUG%" == "Y" @ECHO OFF
...
:BUILDCANDIDATEFILES
IF "%VERBOSE%" == "Y" ECHO.Looking for extension: %~1

@IF "%DEBUG%" == "Y" @ECHO ON
DIR "*.%~1" /b 2>NUL | SED -e "s/$/:%~2/" >> "%UNIQUETEMPFILENAME%"
@IF "%DEBUG%" == "Y" @ECHO OFF
