...
SET PUBLISHCMD=%SCRIPTPATH%\SQLPublishDb.cmd
...
FOR %%F IN (%FOLDER%\*%BACKUPEXTENSION%) DO (
    ECHO.
    ECHO.Publishing: [%%~F]
    CALL "%PUBLISHCMD%" "%%~F" -S "%SERVER%" -U "%USER%" -P "%PASS%" -e "%EDITION%" -su "%SINGLEUSER%" -aip "%ALLOWINCOMPATIBLEPLATFORM%"
)
...
:USAGE
ECHO.%SCRIPTNAME% - Publish all %BACKUPEXTENSION% files in a folder into Databases on a server
ECHO.
ECHO.Usage:
